  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: responseData.message || "Failed to fetch doctor data",
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          message: responseData.message || "Invalid response from doctor API",
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: "An unexpected error occurred while fetching doctor data",
      },
      { status: 500 }
    );
  }
