  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: responseData.message || "Failed to fetch customer data",
    ...
    if (!responseData.data) {
      return NextResponse.json(
        {
          success: false,
          message: responseData.message || "No data returned from customer API",
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: "An unexpected error occurred while fetching customer data",
      },
      { status: 500 }
    );
  }
