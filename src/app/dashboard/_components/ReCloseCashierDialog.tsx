...
const ReCloseCashierDialog: FC<ReCloseCashierDialogProps> = ({
  ...
  const [cashierActivitiesData, setCashierActivitiesData] = useState<any>(null);
  ...
  // Success and Error Alert functions
  const showSuccessAlert = (message: string) => {
    Swal.fire({
      icon: "success",
      title: "Re-Close Cashier Success",
      text: message,
      timer: 3000,
      ...
      switch (e.key) {
        ...
        case "ArrowDown":
          ...
          setSelectedIndex((prevIndex) => {
            ...
            const newIndex = Math.min(prevIndex + 1, activitiesList.length - 1);
        ...
        case "Enter":
          e.preventDefault();
          if (selectedIndex >= 0 && selectedIndex < activitiesList.length) {
            handleRowClick(activitiesList[selectedIndex], selectedIndex);
          }
          break;
  ...
  const handleSubmitWithLogout = async () => {
    if (!selectedItem) {
      ...
      showErrorAlert("Please select a cashier activity to re-close");
      return;
    }

    if (!deviceId) {
      console.error("❌ Device ID not available for reclosing cashier");
      showErrorAlert("Device ID not available for reclosing cashier");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        showSuccessAlert("Cashier activity has been re-closed successfully!");

        // Wait for the success alert to show, then proceed
        setTimeout(async () => {
          onSubmit();
          await logout();
        ...
        showErrorAlert(result.error || "Failed to re-close cashier activity");
      }
    } catch (error) {
      console.error("❌ Error during reclose cashier process:", error);
      showErrorAlert("An unexpected error occurred while re-closing cashier");
    }
  };
