...
const CloseCashierDialog: FC<CloseCashierDialogProps> = ({
  ...
  const checkTokenStatus = async () => {
    ...
    if (!deviceId) {
      console.log("⚠️ Device ID not available, refetching system info...");
  ...
  const handleLoginSuccess = async (userData: any) => {
    ...
    if (deviceId) {
      try {
        ...
        if (result.success) {
          setCashierActivityData(result.data);
          console.log("✅ Cashier activity data refreshed after login");
        }
      } catch (error) {
        console.error(
  ...
  const handleSubmitWithLogout = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showSuccessAlert("Cashier activity has been closed successfully!");
        ...
        showErrorAlert(result.error || "Failed to close cashier activity");
      }
    } catch (error) {
      console.error("❌ Error during close cashier process:", error);
      showErrorAlert("An unexpected error occurred while closing cashier");
    }
  };
                    ...
                    >
                      ...
                      <td className="h-[48px] px-4 text-sm text-gray-600 whitespace-nowrap overflow-hidden text-ellipsis">
                        {filteredData?.tanggal
                          ? new Date(filteredData.tanggal).toLocaleDateString(
                              "en-GB"
                            )
                          : "-"}
                      </td>
