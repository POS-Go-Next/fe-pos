...
const CloseCashierDialog: FC<CloseCashierDialogProps> = ({
    ...
    const filterCashierData = (data: any, searchTerm: string) => {
        ...
        // Since we only have one cashier activity, we filter based on the cashier info
        const matchesSearch =
    ...
    // Apply search filter whenever searchTerm or cashierActivityData changes
    useEffect(() => {
    ...
    const handleLoginSuccess = async (userData: any) => {
        ...
        // Refetch cashier activity data after login
        if (deviceId) {
            try {
                const result = await checkCashierActivity(deviceId);
                if (result.success) {
