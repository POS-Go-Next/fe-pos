...
export const useParameter = (): UseParameterReturn => {
  const [parameterData, setParameterData] = useState<ParameterData | null>(null);
  ...
  const fetchParameter = async (force: boolean = false) => {
    try {
      ...
      console.log('🔄 Fetching parameter data...', { force, timestamp: new Date().toISOString() });

      // FIXED: Add cache-busting parameter to prevent caching issues
      const cacheBuster = force ? `?_t=${Date.now()}` : '';
      
      // Call our Next.js API route untuk parameter
      const response = await fetch(`/api/parameter/me${cacheBuster}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          // FIXED: Add no-cache headers to ensure fresh data
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch parameter data');
  ...
  const updateParameter = async (updatedData: Partial<ParameterData>): Promise<boolean> => {
    try {
      setError(null);
      ...
      // FIXED: Immediately update local state with the new data
      if (parameterData) {
        const updatedParameterData = { ...parameterData, ...updatedData };
        console.log('🔄 Updating local state immediately:', {
          before: { kd_area: parameterData.kd_area, kd_cab: parameterData.kd_cab },
          after: { kd_area: updatedParameterData.kd_area, kd_cab: updatedParameterData.kd_cab }
        });
        setParameterData(updatedParameterData);
      }

      // FIXED: Force refetch with cache busting after a short delay
      setTimeout(async () => {
        console.log('🔄 Force refreshing parameter data...');
