...
export const useRecloseActivity = (): UseRecloseActivityReturn => {
  ...
  const [lastResponse, setLastResponse] = useState<RecloseActivityData | null>(
    null
  );
  ...
  const recloseActivity = async (
    ...
    try {
      ...
      if (result.data) {
        ...
        console.log("✅ Cashier activity reclosed successfully:", result.data);
        return {
          success: true,
          isSessionExpired: false,
          data: result.data,
