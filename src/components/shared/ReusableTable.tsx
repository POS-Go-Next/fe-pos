      ...
      {Array.from({ length: loadingRows }).map((_, index) => (
        <tr key={`loading-${index}`} className="border-b border-gray-100">
          {columns.map((column, colIndex) => (
            <td key={`loading-${index}-${colIndex}`} className="p-3">
        ...
        {visibleActions.map((action, actionIndex) => {
          ...
          switch (action.type) {
            ...
            case "custom":
              ...
              buttonClassName = action.className || buttonClassName;
              break;
          }
  ...
  return (
    ...
    >
      <div className="relative">
              ...
              {loading
                ...
                : data.map((row, index) => {
                    const isSelected =
                      ...
                      (row.id === selectedRowId || index === selectedRowId);
        ...
        {/* Fixed Action Column - Alternative Implementation */}
